...
// ============================================================================
    ...
    void onReadError(Throwable t) {
        ...
        if (t instanceof EOFException) {
            debug.log( "onReadError: received EOF");
            eof = (EOFException) t;
        }
        CompletableFuture<?> cf = receiver == null ? null : receiver.completion();
        debug.log( "onReadError: cf is "
                + (cf == null  ? "null"
                : (cf.isDone() ? "already completed"
                               : "not yet completed")));
        if (cf != null) {
            ...
            debug.log( "onReadError", t);
        }
        debug.log( "closing connection: cause is " + t);
        connection.close();
    }
    ...
    Receiver<?> receiver(State state) {
        Receiver<?> receiver;
        switch (state) {
            case READING_HEADERS    :receiver= headersReader;break;
            case READING_BODY       : receiver=bodyReader;break;

...
public class FramesEncoder {
    ...
    public List<ByteBuffer> encodeFrame(Http2Frame frame) {
        ...
        switch (frame.type()) {
            case DataFrame.TYPE :          data=encodeDataFrame((DataFrame) frame);break;
            case HeadersFrame.TYPE :       data=encodeHeadersFrame((HeadersFrame) frame);break;
            case PriorityFrame.TYPE :      data=encodePriorityFrame((PriorityFrame) frame);break;
            case ResetFrame.TYPE :         data=encodeResetFrame((ResetFrame) frame);break;
            case SettingsFrame.TYPE :      data=encodeSettingsFrame((SettingsFrame) frame);break;
            case PushPromiseFrame.TYPE :   data=encodePushPromiseFrame((PushPromiseFrame) frame);break;
            case PingFrame.TYPE :          data=encodePingFrame((PingFrame) frame);break;
            case GoAwayFrame.TYPE :        data=encodeGoAwayFrame((GoAwayFrame) frame);break;
            case WindowUpdateFrame.TYPE :  data=encodeWindowUpdateFrame((WindowUpdateFrame) frame);break;
            case ContinuationFrame.TYPE :  data=encodeContinuationFrame((ContinuationFrame) frame);break;
    ...
    private List<ByteBuffer> joinWithPadding(ByteBuffer buf, List<ByteBuffer> data, int padLength) {
        int len = data.size();
        if (len == 0) return Arrays.asList(buf, getPadding(padLength));
        else if (len == 1) return Arrays.asList(buf, data.get(0), getPadding(padLength));
        else if (len == 2) return Arrays.asList(buf, data.get(0), data.get(1), getPadding(padLength));
    ...
    private List<ByteBuffer> join(ByteBuffer buf, List<ByteBuffer> data) {
        int len = data.size();
        if (len == 0) return Collections.singletonList(buf);
        else if (len == 1) return Arrays.asList(buf, data.get(0));
        else if (len == 2) return Arrays.asList(buf, data.get(0), data.get(1));

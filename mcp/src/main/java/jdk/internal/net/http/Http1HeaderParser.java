...
class Http1HeaderParser {
    ...
    boolean parse(ByteBuffer input) throws ProtocolException {
        ...
        while (canContinueParsing(input)) {
            switch (state) {
                case INITIAL                                    :  state = State.STATUS_LINE;break;
                case STATUS_LINE                                :  readResumeStatusLine(input);break;
                case STATUS_LINE_FOUND_CR                       :  readStatusLineFeed(input);break;
                case STATUS_LINE_FOUND_LF                       :  readStatusLineFeed(input);break;
                case STATUS_LINE_END                            :  maybeStartHeaders(input);break;
                case STATUS_LINE_END_CR                         :  maybeEndHeaders(input);break;
                case STATUS_LINE_END_LF                         :  maybeEndHeaders(input);break;
                case HEADER                                     :  readResumeHeader(input);break;
                case HEADER_FOUND_CR                            :  resumeOrLF(input);break;
                case HEADER_FOUND_LF                            :  resumeOrLF(input);break;
    ...
    private boolean canContinueParsing(ByteBuffer buffer) {
        ...
        boolean canContinue;
        switch (state) {
            case FINISHED : canContinue=false;break;
            case STATUS_LINE_FOUND_LF :canContinue= true;break;
            case STATUS_LINE_END_LF :canContinue= true;break;
            case HEADER_FOUND_LF :canContinue=true;break;
            default : canContinue=buffer.hasRemaining();
        };
        return canContinue;
    }

...
class Http2ClientImpl {
    ...
    final static Logger debug =
            LoggerFactory.getLogger(Http2ClientImpl.class);
    ...
    CompletableFuture<Http2Connection> getConnectionFor(HttpRequestImpl req,
        ...
        synchronized (this) {
                ...
                try {
                    if (connection.closed || !connection.reserveStream(true)) {
                        if (debug.isDebugEnabled())
                            debug.debug("removing found closed or closing connection: %s", connection);
                        deleteConnection(connection);
                    } else {
                        // fast path if connection already exists
                        if (debug.isDebugEnabled())
                            debug.debug("found connection in the pool: %s", connection);
            ...
            if (!req.secure() || failures.contains(key)) {
                ...
                if (debug.isDebugEnabled()) debug.debug("not found in connection pool");
    ...
    boolean offerConnection(Http2Connection c) {
        if (debug.isDebugEnabled()) debug.debug("offering to the connection pool: %s", c);
        if (c.closed || c.finalStream()) {
            if (debug.isDebugEnabled())
                debug.debug("skipping offered closed or closing connection: %s", c);
            return false;
        }
        ...
        synchronized(this) {
            ...
            if (c1 != null) {
                c.setFinalStream();
                if (debug.isDebugEnabled())
                    debug.debug("existing entry in connection pool for %s", key);
                return false;
            }
            if (debug.isDebugEnabled())
                debug.debug("put in the connection pool: %s", c);
            return true;
        }
    }
    ...
    void deleteConnection(Http2Connection c) {
        if (debug.isDebugEnabled())
            debug.debug("removing from the connection pool: %s", c);
        synchronized (this) {
            Http2Connection c1 = connections.get(c.key());
            if (c1 != null && c1.equals(c)) {
                connections.remove(c.key());
                if (debug.isDebugEnabled())
                    debug.debug("removed from the connection pool: %s", c);
            }
        }
    }
    ...
    void stop() {
        if (debug.isDebugEnabled()) debug.debug("stopping");

...
public class RawChannelTube implements RawChannel {
    ...
    static final class CleanupChecker implements Runnable {
        ...
        CleanupChecker(AtomicBoolean closed) {
            this.closed = closed;
        }
    ...
    private static List<ByteBuffer> copy(ByteBuffer[] src, int offset, int len) {
        ...
        if (count == 1) return Collections.singletonList(Utils.copy(src[offset]));
        if (count == 2) return Arrays.asList(Utils.copy(src[offset]), Utils.copy(src[offset+1]));

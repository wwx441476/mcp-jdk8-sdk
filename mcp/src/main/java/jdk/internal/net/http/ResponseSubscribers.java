...
public class ResponseSubscribers {
        ...
        implements TrustedSubscriber<InputStream>
    {
        ...
        private static final List<ByteBuffer> LAST_LIST = Collections.singletonList(LAST_BUFFER);
        private static final Logger debug =
                LoggerFactory.getLogger(HttpResponseInputStream.class);
            ...
            while (currentBuffer == null || !currentBuffer.hasRemaining()) {
                ...
                try {
                    if (currentListItr == null || !currentListItr.hasNext()) {
                        ...
                        if (debug.isDebugEnabled()) debug.debug("Taking list of Buffers");
                        List<ByteBuffer> lb = buffers.take();
                        currentListItr = lb.iterator();
                        if (debug.isDebugEnabled()) debug.debug("List of Buffers Taken");
                        ...
                        if (s != null) {
                            if (debug.isDebugEnabled()) debug.debug("Increased demand by 1");
                            s.request(1);
                        }
                    ...
                    if (debug.isDebugEnabled()) debug.debug("Next Buffer");
            ...
            try {
                if (!subscribed.compareAndSet(false, true)) {
                    ...
                    if (debug.isDebugEnabled())
                        debug.debug("onSubscribe: requesting "
        ...
        public void onNext(List<ByteBuffer> t) {
            ...
            try {
                if (debug.isDebugEnabled()) debug.debug("next item received");
                if (!buffers.offer(t)) {
                    throw new IllegalStateException("queue is full");
                }
                if (debug.isDebugEnabled()) debug.debug("item offered");
            } catch (Throwable ex) {
                failed = ex;
                try {
                    close();

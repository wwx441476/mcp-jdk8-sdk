...
abstract class HttpConnection implements Closeable {
    ...
    final Logger debug = Logger.getLogger(HttpConnection.class);
    final static Logger DEBUG_LOGGER = Logger.getLogger(HttpConnection.class);
    ...
    final boolean checkOpen() {
        if (isOpen()) {
            try {
                ...
                debug.debug("Pooled connection is no longer operational: %s",
                        x.toString());
                return false;
            }
        }
   ...
   private static final boolean hasRequiredHTTP2TLSVersion(HttpClient client) {
        ...
        if (!secure) {
            ...
            if (c != null && c.checkOpen() /* may have been eof/closed when in the pool */) {
                ...
                if (DEBUG_LOGGER.isDebugEnabled())
                    DEBUG_LOGGER.debug(conn.getConnectionFlow()
    ...
    final class PlainHttpPublisher implements HttpPublisher {
        ...
        final class HttpWriteSubscription implements Flow.Subscription {
            ...
            public void request(long n) {
                ...
                if (debug.isDebugEnabled())
                    debug.debug("HttpPublisher: got request of "  + n + " from "
                               + getConnectionFlow());
                writeScheduler.runOrSchedule();
            }
            ...
            public void cancel() {
                if (debug.isDebugEnabled())
                    debug.debug("HttpPublisher: cancelled by " + getConnectionFlow());
            }
            ...
            void flush() {
                while (!isEmpty() && demand.tryDecrement()) {
                    ...
                    if (debug.isDebugEnabled())
                        debug.debug("HttpPublisher: sending "
        ...
        public void enqueue(List<ByteBuffer> buffers) throws IOException {
            ...
            debug.debug("added %d bytes to the write queue", bytes);
        }
        ...
        public void enqueueUnordered(List<ByteBuffer> buffers) throws IOException {
            ...
            debug.debug("added %d bytes in the priority write queue", bytes);
        }

        @Override
        public void signalEnqueued() throws IOException {
            debug.debug("signalling the publisher of the write queue");
            signal();
        }
    }

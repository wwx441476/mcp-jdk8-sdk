...
final class SocketTube implements FlowTube {
    ...
    final Logger debug = Logger.getLogger(getClass());
    ...
    static final List<ByteBuffer> EOF = Collections.emptyList();
    static final List<ByteBuffer> NOTHING = Collections.singletonList(Utils.EMPTY_BYTEBUFFER);
    ...
    private static <T> List<T> listOf(List<T> list, T item) {
        ...
        switch (size) {
            case 0: return Collections.singletonList(item);
            case 1: return Arrays.asList(list.get(0), item);
            case 2: return Arrays.asList(list.get(0), list.get(1), item);
